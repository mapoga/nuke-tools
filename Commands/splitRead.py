...
def split_selected_reads_panel():
	import splitReadDialog as srd
	#Mixed layers
	mixed_layers = []
	for n in nuke.selectedNodes():
		if n.Class() == 'Read':
			...
			#selected_layers = [l for l in layers if l.startswith('beauty_aux_')]
			mixed_layers.extend(layers)

	mixed_layers = list(set(mixed_layers))
	mixed_layers.sort()

	#Dialog
	dialog = srd.LayersDialog(mixed_layers)
	#Filter enabled
	enabled = []
	for l in mixed_layers:
		if l.startswith('beauty_aux_') or l.startswith('bty_') or l.startswith('volume_') or l.startswith('vol_'):
			enabled.append(True)
		else:
			enabled.append(False)

	for idx, x in enumerate(range(dialog.layers_widget.count())):
		item = dialog.layers_widget.item(x)
		item.set_enabled(enabled[idx])


	dialog.show()
	if dialog.exec_() == True:
		layers = dialog.create()
		print(layers)
		...
		for n in nuke.selectedNodes():
			if n.Class() == 'Read':
				split_layers(n, layers, Redshift_crypto=True)
